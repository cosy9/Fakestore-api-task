...
if ((!page_path.includes('Category'))&&(!page_path.includes('Cart'))) { // checking if url is not category or cart
...
function categoryProducts(categoryText) { // to load all category
//1 initialize AJAX
var xhr = new XMLHttpRequest()
//2 open object
...
if(page_path_category.includes('Category.html')){
  ...
  selectSort.addEventListener('change',function(e){ // For sorting product category 
  e.preventDefault()
...
function manageQuantity(){
  ...
  for (var i = 0; i < decreaseButtons.length; i++) {
    decreaseButtons[i].addEventListener('click',function(){
      ...
      currentProductId = this.parentElement.nextElementSibling.nextElementSibling.getAttribute('data-attribute') //to get data-attribute from anchor which has ID stored in in
