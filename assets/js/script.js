...
if (!page_path.includes('Category') && !page_path.includes('Cart')) {
  // checking if url is not category or cart
    ...
    xhr.onload = function () {
      if (this.status === 200) {
        ...
        for (var i = 0; i < object.length; i++) {
          ...
          var html =
            '<li class="product not-active-list"><figure><img src=' +
            image +
            ' alt=' +
            title +
            '></figure><h5>' +
            ...
            '</h6><p class="desc">' +
            description +
            '</p><span title="Read More" class="readmore" onclick = "ReadMore(this)" >Read More</span><p>Price : ' +
            price +
            ' $ </p><a class="product-id" href="#FIXME" data-attribute=' +
            id +
  ...
  function loadMore() {
    var elementList = Array.from(
      document.querySelectorAll('.all-products-data .product')
    )
    for (let i = 0; i <= 4; i++) {
      if (elementList[i]) {
  ...
  inputData.addEventListener('input', showSpecificData) //filtering data according to input by user
  ...
  setTimeout(() => {
    ...
    // console.log(desc)
    desc.forEach((content) => {
      if (content.textContent.length < noOfCharac) {
        content.nextElementSibling.style.display = 'none'
      } else {
        let displayText = content.textContent.slice(0, noOfCharac)
        let moreText = content.textContent.slice(noOfCharac) // no of characters after 150 characters.
        // everything inside a para.
...
function categoryProducts(categoryText) {
  // to load all category
  //1 initialize AJAX
  var xhr = new XMLHttpRequest()
  //2 open object
  xhr.open(
    'GET',
    'https://fakestoreapi.com/products/category/' + categoryText + '',
    true
  )
  //3 on load object
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      var categoryTextTrim = categoryText
        .replace(/'/g, '')
        .replace(/ /g, '')
        .trim()
      var inputHtml = document.querySelector(
        '.' + categoryTextTrim + '-products'
      )
      inputHtml.innerHTML = ''
      for (var i = 0; i < object.length; i++) {
        var category = object[i]['category']
        ...
        price = parseInt(price)
        var html =
          '<li class="product"><figure><img src=' +
          image +
          ' alt=' +
          category +
          '></figure><h5>' +
...
if (page_path_category.includes('Category.html')) {
  ...
  selectSort.addEventListener('change', function (e) {
    // For sorting product category
    e.preventDefault()
    var seasonValue = document.querySelector("select[name='sort-product']")
    var seasonVal =
      seasonValue.options[seasonValue.selectedIndex].text.toLowerCase()
    if (seasonVal == 'all') {
...
setTimeout(function () {
  ...
  for (let l = 0; l < allProductList.length; l++) {
    ...
    allProductList[l]['price'] = parseInt(allProductList[l]['price'])
  }
...
function productAdded() {
  var popDiv = document.querySelector('.pop-up')
  popDiv.classList.remove('not-active')
  setTimeout(function () {
    popDiv.classList.add('not-active')
  }, 900)
}
...
//storing data to local storage via product ID
...
function setItems(product) {
  ...
  if (cartItems != null) {
    // checking if cart item is not null.
    if (cartItems[product.id] == undefined) {
      // then checking if product does not exist already
      cartItems = {
        ...cartItems, // here 1 is product.id and : rest is product
        [product.id]: product, // then appending the new product productsInCart:"{"1":{"id":1,"title":"Fjallraven - Foldsack No. 1 Backpack, Fits 15 Laptops","price":109,"description":"Your perfect pack for everyday use and walks in the forest. Stash your laptop (up to 15 inches) in the padded sleeve, your everyday","category":"men's clothing","image":"https://fakestoreapi.com/img/81fPKd-2AYL._AC_SL1500_.jpg","rating":{"rate":3.9,"count":120},"incart":1}}"
      }
    }
    cartItems[product.id].incart += 1 // if product already exist then increment incart value
  } else {
    product.incart = 1 // if the product is been added first time
    cartItems = {
      [product.id]: product,
    }
  }
  localStorage.setItem('productsInCart', JSON.stringify(cartItems)) //setting cartItem value to local storage
}
...
// checks local storage for number of products in cart each time page reloads
...
//displaying added/updated data in cart page
  ...
  if (cartItems && productContainer) {
    ...
    Object.values(cartItems).forEach(function (val) {
      ...
      productContainer.innerHTML +=
        ...
        '</figcaption></figure><p>$ ' +
        val.price +
        '</p><p><span class="left">left</span>' +
        val.incart +
        '<span class="right">right</span></p><p>$ ' +
        val.price * val.incart +
        '</p><a class="removeBtn" href= "#fixme" data-attribute="' +
        val.id +
        '" title ="Remove">Remove</a></li>'
    })

    productContainer.innerHTML +=
      '<li><h4>Cart Total</h4><p>$' + cartCost + '</p></li>'
  }
  deleteButtons()
  manageQuantity()
}
...
function deleteButtons() {
  ...
  var productId
  var cartItems = localStorage.getItem('productsInCart') // object of products
  var productNumbers = localStorage.getItem('cartNumbers') // cart value
  var cartCost = localStorage.getItem('totalCost') // total cost
...
function manageQuantity() {
  ...
  var cartItems = localStorage.getItem('productsInCart') // objects
  ...
  for (var i = 0; i < decreaseButtons.length; i++) {
    decreaseButtons[i].addEventListener('click', function () {
      ...
      currentProductId =
        this.parentElement.nextElementSibling.nextElementSibling.getAttribute(
          'data-attribute'
        ) //to get data-attribute from anchor which has ID stored in in
